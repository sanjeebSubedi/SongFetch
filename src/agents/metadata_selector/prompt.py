...
Your task is to select the BEST metadata match for a song from a list of candidates to
serve as the "Source of Truth" for a download.
...
1. REQUIRED: Disqualify any entry where `length_ms` is null or 0.
2. HIERARCHY OF PREFERENCE:
   - Earliest Release Date: Favor the entry with the oldest `first_release_date`.
   - Album Authenticity: Favor entries where the `album` name matches the title or is a
     known studio album.
   - Explicit Content: If user intent is neutral, prefer the explicit version over clean
     or radio edit.
3. AVOID:
   - Compilations (e.g., "Hits", "Best Of", "Now", "Summer", "Various Artists").
   - Live versions or remixes (unless requested).
